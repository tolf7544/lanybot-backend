...
export function skipEmbed(stream:Metadata,server:CommunityServer) {
	const lang = new Lang(server.guild.id)
	const embed = new EmbedBuilder()
...
export function loopEmbed(server:CommunityServer, input:string,metadata:Metadata) {
	const lang = new Lang(server.guild.id)
	let title = '';
	let description = '';
	if(input == 'single') {
		title = lang.loopSingle_Title;
		description = metadata.MusicData.title 
	} else {
...
export function clearSystemMsg(common: 'common'|'loop'|'error'|'error1',lang:Lang,GuildId:string | undefined,metadata:Metadata | undefined): void {
	if(!GuildId || !metadata) return;
	...
	const embed = new EmbedBuilder()
		...
		.setDescription(`${lang.clearSystemMsg_Description}:${metadata.MusicData.title + " "}\n`)
		.setImage(`${metadata.MusicData.thumbnail}`)
		.setColor(`Green`)
	...
	const embedE = new EmbedBuilder()
		...
		.setDescription(`${lang.clearSystemMsg_embedE_Description[0]} **[${metadata.MusicData.title}]** ${lang.clearSystemMsg_embedE_Description[1]}\n`)
		.setThumbnail(`${metadata.MusicData.thumbnail}`)
		.setColor(`Orange`)
		.addFields({name:'Tip_',value:`${lang.clearSystemMsg_embedE_Footer[0]} [youtube Link](${'https://youtu.be/' + metadata.id}) ${lang.clearSystemMsg_embedE_Footer[1]} `})
		.setFooter({text:communityServer.member.displayName})
	...
	const embedE1 = new EmbedBuilder()
		...
		.setThumbnail(`${metadata.MusicData.thumbnail}`)
		.setColor(`Red`)
		.addFields({name:'Tip_',value:`[youtube Link](${'https://youtu.be/' + metadata.id})`})
		.setFooter({text:communityServer.member.displayName})
		try {
		if (common == 'error1') {
